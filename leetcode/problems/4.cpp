...
public:
    double findMedianSortedArrays(vector<int>& nums1, vector<int>& nums2) {
        ...
        while (left <= right) {
            ...
            int left_max_1 = p1 -1 < 0 ? INT_MIN: nums1[p1 - 1];
            // Value of the first right side element in nums1 
            int right_min_1 = p1 >= n1 ? INT_MAX : nums1[p1];
            // Value of the last left side element in nums2 
            int left_max_2 = p2 - 1 < 0 ? INT_MIN: nums2[p2 - 1];
            // Value of the first right side element in nums2 
            int right_min_2 = p2 >= n2 ? INT_MAX: nums2[p2];
            if (left_max_1 > right_min_2) {
                ...
                if ((n1 + n2) % 2 == 1) { // odd
                    ...
                    return 1.0 * (max(left_max_1, left_max_2) + min(right_min_1, right_min_2)) / 2;
                }
            }
        }
